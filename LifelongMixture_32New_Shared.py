...
def autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, task_state, Component_Count):
    ...
    reco1_loss = reco1 * task_state[0, 0]
    reco2_loss = reco2 * task_state[0, 1]
    reco3_loss = reco3 * task_state[0, 2]
    reco4_loss = reco4 * task_state[0, 3]

    # loss
    marginal_likelihood = (reco1_loss + reco2_loss + reco3_loss + reco4_loss) / (
                task_state[0, 0] + task_state[0, 1] + task_state[0, 2] + task_state[0, 3])
    ...
    KL_divergence = k1 * task_state[0, 0] + k2 * task_state[0, 1] + k3 * task_state[0, 2] + k4 * task_state[0, 3]
    KL_divergence = KL_divergence / (task_state[0, 0] + task_state[0, 1] + task_state[0, 2] + task_state[0, 3])
...
with tf.Session(config=config) as sess:
    ...
    if isWeight:
        saver.restore(sess, 'models/LifelongMixture_32New_Shared')
    ...
    saver.save(sess, 'models/LifelongMixture_32New_Shared')
