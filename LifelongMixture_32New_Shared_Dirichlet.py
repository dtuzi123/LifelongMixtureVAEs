...
def autoencoder(x_hat, x, dim_img, dim_z, n_hidden, keep_prob, task_state, Component_Count):
    ...
    s1 = Generator_SharedSVHN(z1, "sharedDecoder", reuse=isUsed)
    s2 = Generator_SharedSVHN(z2, "sharedDecoder", reuse=True)
    s3 = Generator_SharedSVHN(z3, "sharedDecoder", reuse=True)
    s4 = Generator_SharedSVHN(z4, "sharedDecoder", reuse=True)

    x1 = Generator_SubSVHN(s1, "decoder1", reuse=isUsed)
    x2 = Generator_SubSVHN(s2, "decoder2", reuse=isUsed)
    x3 = Generator_SubSVHN(s3, "decoder3", reuse=isUsed)
    x4 = Generator_SubSVHN(s4, "decoder4", reuse=isUsed)
...
def MyTest():
    ...
    with tf.Session(config=config) as sess:
        ...
        saver.restore(sess, 'models/LifelongMixture_32New_Shared_Dirichlet')
        currentTask = 4
        ...
        for taskIndex in range(currentTask):
            ...
            if index == 0:
                ...
                z1 = mu1 + sigma1 * tf.random_normal(tf.shape(mu1), 0, 1, dtype=tf.float32)
                s1 = Generator_SharedSVHN(z1, "sharedDecoder", reuse=True)
                reco = Generator_SubSVHN(s1, "decoder1", reuse=True)
            elif index == 1:
                mu2, sigma2 = Encoder_SVHN(x_hat, "encoder2", reuse=True)
                z2 = mu2 + sigma2 * tf.random_normal(tf.shape(mu2), 0, 1, dtype=tf.float32)
                s1 = Generator_SharedSVHN(z2, "sharedDecoder", reuse=True)
                reco = Generator_SubSVHN(s1, "decoder2", reuse=True)
            elif index == 2:
                mu3, sigma3 = Encoder_SVHN(x_hat, "encoder3", reuse=True)
                z3 = mu3 + sigma3 * tf.random_normal(tf.shape(mu3), 0, 1, dtype=tf.float32)
                s1 = Generator_SharedSVHN(z3, "sharedDecoder", reuse=True)
                reco = Generator_SubSVHN(s1, "decoder3", reuse=True)
            elif index == 3:
                mu4, sigma4 = Encoder_SVHN(x_hat, "encoder4", reuse=True)
                z4 = mu4 + sigma4 * tf.random_normal(tf.shape(mu4), 0, 1, dtype=tf.float32)
                s1 = Generator_SharedSVHN(z4, "sharedDecoder", reuse=True)
                reco = Generator_SubSVHN(s1, "decoder4", reuse=True)
...
with tf.Session(config=config) as sess:
    ...
    if isWeight:
        ...
        saver.save(sess, 'models/LifelongMixture_32New_Shared_Dirichlet')
